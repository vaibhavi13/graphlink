...
def kmeans_python(points, k, max_iters):
    ...
    for iter in range(max_iters):
        ...
        threads_per_block = 256
        blocks_per_grid = (points.shape[0] + threads_per_block - 1) // threads_per_block
        kmeans_kernel[blocks_per_grid, threads_per_block](points, centroids, cluster_assignments)

        # Compute new centroids
        for i in range(k):
            mask = (cluster_assignments == i)
            if cp.any(mask):
                centroids[i] = cp.mean(points[mask], axis=0)
...
#Using CUDA with C++
cluster_assignments = kmeans(cp.asarray(points), k, max_iters)

#Using CUDA with Python
# cluster_assignments = kmeans_python(cp.asarray(points), k, max_iters)
